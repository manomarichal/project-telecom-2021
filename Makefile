...
install-local: $(ELEMENTMAP) $(CLICK_BUILDTOOL) $(CLICK_COMPILE) \
	...
	$(INSTALL_IF_CHANGED) $(top_srcdir)/test/clicktest $(DESTDIR)$(bindir)/clicktest
...
uninstall-local:
	/bin/rm -f $(DESTDIR)$(bindir)/click-buildtool $(DESTDIR)$(bindir)/click-compile $(DESTDIR)$(bindir)/click-mkelemmap $(DESTDIR)$(bindir)/testie $(DESTDIR)$(bindir)/clicktest $(DESTDIR)$(clickdatadir)/elementmap.xml $(DESTDIR)$(clickdatadir)/srcdir $(DESTDIR)$(clickdatadir)/src $(DESTDIR)$(clickdatadir)/config.mk $(DESTDIR)$(clickdatadir)/mkinstalldirs
...
check: $(ALL_TARGETS) Makefile $(ELEMENTMAP)
	$(top_srcdir)/test/clicktest -p $(top_builddir)/bin -p $(top_srcdir)/test \
...
distdir: $(srcdir)/configure
	-rm -rf doc/click.info* doc/clicktest.1 $(srcdir)/doc/click.info* $(srcdir)/doc/clicktest.1
	cd doc && $(MAKE) clicktest.1
	cd $(srcdir)/etc/samplepackage && $(AUTOCONF)
	-rm -rf $(distdir)
	mkdir $(distdir)
	chmod 777 $(distdir)
	@echo Copying library, documentation, configuration, and driver files...
	[ "$(srcdir)" = . ] || cp doc/clicktest.1 $(srcdir)/doc
