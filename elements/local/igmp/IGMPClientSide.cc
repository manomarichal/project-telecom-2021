...
WritablePacket * IGMPClientSide::make_mem_report_packet()
{
    ...
    WritablePacket *p = Packet::make(igmp_get_size_of_data(group_records) + sizeof(click_ip) + 4);
    memset(p->data(), 0, p->length()); // erase previous random data on memory requested

    click_ip *ip_header = igmp_add_ip_header(p, clientIP, MC_ADDRESS, true);
    router_alert* r_alert = igmp_add_router_alert(ip_header+1);
    igmp_add_igmp_data(r_alert+1, group_records);

    p->set_ip_header(ip_header, sizeof(click_ip));
    p->timestamp_anno().assign_now();
    p->set_dst_ip_anno(IPAddress(MC_ADDRESS));

    click_chatter("\t igmp data size: %i", igmp_get_size_of_data(group_records));
