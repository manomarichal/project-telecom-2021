...
uint32_t igmp_get_size_of_data(const Vector<igmp_group_record> group_records)
{
    int32_t size = 0;
...
click_ip* igmp_add_ip_header(WritablePacket* p, IPAddress client_ip, IPAddress multicast_address, bool verbose)
{
...
router_alert * igmp_add_router_alert(void *start, uint8_t octet_1=0, uint8_t octet_2=0)
{
    // add ip option
...
igmp_mem_report* igmp_add_igmp_data(void *start, const Vector<igmp_group_record> group_records)
{
